...
public class TcElementImpl extends EObjectImpl implements TcElement {
	...
	public void attachAsChildren(ListOfSubtree inListOfSubtrees) {
		for (TcElement curSubtree : inListOfSubtrees.getList()) {
			getChildren().add(curSubtree.deepCopy());
		}
	}
	...
	public TcElement deepCopy() {
		...
		for (TcAttribute curAttribute : getAttributes()) {
			result.getAttributes().add(curAttribute.deepCopy(result.getObjectId()));
		}
		for (TcElement curChild : getChildren()) {
			result.getChildren().add(curChild.deepCopy());
			}
